...
describe('sendMessage handler', () => {
  ...
  it('should return "bad request" when no payload', async () => {
    await expect(callHandler(handler, { connectionId, tokenData: data })).resolves.toEqual(badRequest());
  });

  it('should return "bad request" when message is empty', async () => {
    await expect(callHandler(handler, { connectionId, tokenData: data, body: { message: '' } })).resolves.toEqual(badRequest());
  });
  ...
  it('should let aws errors bubble when it happens on "add" operation', async () => {
    ...
    await expect(callHandler(handler, { connectionId, tokenData: data, body: payload })).resolves.toEqual(serverError());
  });
  ...
  describe('success cases', () => {
    ...
    const ttl = 10000;
    const follower1: ThreadEvent = { pk, time: 1, eventType: 'follow', connectionId, ttl, userId: data.userId };
    ...
    it('should add thread event "message"', async () => {
      await expect(callHandler(handler, { connectionId, tokenData: data, body: payload })).resolves.toEqual(ok());
